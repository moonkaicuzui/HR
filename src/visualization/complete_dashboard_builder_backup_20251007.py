...
class CompleteDashboardBuilder:
    ...
    def _collect_team_data(self):
        """
        Collect team-based data from basic_manpower
        팀별 데이터 수집

        Returns:
            Dict with team structure and metrics
        """
        data = self.collector.load_month_data(self.target_month)
        df = data.get('basic_manpower', pd.DataFrame())
        attendance_df = data.get('attendance', pd.DataFrame())
        ...
        # Parse team structure fields
        # QIP POSITION 1ST NAME: 직급 (ASSEMBLY INSPECTOR, LINE LEADER, etc.)
        # QIP POSITION 2ND NAME: 팀 (SHOES INSPECTOR, UPPER INSPECTOR, etc.)
        # QIP POSITION 3RD NAME: 부서 (ASSEMBLY LINE TQC, etc.)
        ...
        for idx, row in df.iterrows():
            ...
            position_1st = str(row.get('QIP POSITION 1ST  NAME', ''))  # Note: double space
            position_2nd = str(row.get('QIP POSITION 2ND  NAME', ''))  # Note: double space
            position_3rd = str(row.get('QIP POSITION 3RD  NAME', ''))

            # Boss ID mapping - MST direct boss name is Employee No stored as float
            boss_id = ''
            ...
            # Group by 1st position (primary grouping)
            if position_1st and position_1st != 'nan':
                if position_1st not in team_data:
                    team_data[position_1st] = {
                        'name': position_1st,
                        'members': [],
                        'sub_teams': {}  # 2nd level teams
                    }
    ...
    <div class="modal-dialog modal-xl">
        <div class="modal-content">
            <div class="modal-header">
                <h5 class="modal-title lang-modal-title" data-ko="총 재직자 수" data-en="Total Employees" data-vi="Tổng số nhân viên">총 재직자 수</h5>
                <div class="d-flex align-items-center gap-2">
                    <select class="form-select form-select-sm" id="modalTeamFilter1" onchange="filterModalByTeam(1)" style="width: 200px;">
            ...
            <div class="modal-body">
                ...
                <p class="lang-calc-desc" data-ko="재직자 = 퇴사일이 없거나 퇴사일이 월말 이후인 직원"
                   data-en="Active Employees = Employees with no stop date or stop date after month end"
                   data-vi="Nhân viên đang làm = Không có ngày nghỉ hoặc ngày nghỉ sau cuối tháng">
                   재직자 = 퇴사일이 없거나 퇴사일이 월말 이후인 직원
                </p>
                <hr>
                <div id="modalContent1">
                    ...
                    <table class="table table-sm table-hover modal-table" id="modalTable1">
                        <thead>
                            <tr>
                                <th onclick="sortModalTable(1, 0)" class="lang-th" data-ko="사번" data-en="ID" data-vi="Mã NV">사번 <span class="sort-icon">▼</span></th>
                                <th onclick="sortModalTable(1, 1)" class="lang-th" data-ko="이름" data-en="Name" data-vi="Tên">이름 <span class="sort-icon">▼</span></th>
                                <th onclick="sortModalTable(1, 2)" class="lang-th" data-ko="직급" data-en="Position" data-vi="Vị trí">직급 <span class="sort-icon">▼</span></th>
                                <th onclick="sortModalTable(1, 3)" class="lang-th" data-ko="입사일" data-en="Entrance Date" data-vi="Ngày vào">입사일 <span class="sort-icon">▼</span></th>
                                <th onclick="sortModalTable(1, 4)" class="lang-th" data-ko="재직기간" data-en="Tenure" data-vi="Thâm niên">재직기간 <span class="sort-icon">▼</span></th>
                            </tr>
                        </thead>
                        <tbody id="modalTableBody1"></tbody>
                    </table>
                </div>
            </div>
        </div>
...
function showModal1() {
    ...
    // Populate table
    const tbody = document.getElementById('modalTableBody1');
    tbody.innerHTML = employees.map(e => `
        <tr>
            ...
            <td>${e.tenure_days}일 (${Math.floor(e.tenure_days/30)}개월)</td>
        </tr>
    `).join('');
    ...
    setTimeout(() => {
        if (modalCharts['modal1']) modalCharts['modal1'].destroy();

        const ctx = document.getElementById('modalChart1').getContext('2d');
        modalCharts['modal1'] = new Chart(ctx, {
            type: 'doughnut',
            data: {
                labels: ['재직자 / Active', '퇴사자 / Resigned'],
                datasets: [{
                    data: [
                        employeeDetails.filter(e => e.is_active).length,
                        employeeDetails.filter(e => !e.is_active).length
                    ],
